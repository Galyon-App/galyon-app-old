...
export class ManageOrdersComponent implements OnInit {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe((data) => {
      if (data && data.uuid) {
        if (this.storeService.storeValue && this.storeService.storeValue.name) {
  ...
  getOrder() {
    this.spinner.show();
    this.api.post('galyon/v1/orders/getByID', {
      uuid: this.id
    }).then((response: any) => {
      this.spinner.hide();
      this.loaded = true;
      ...
      if (response && response.success == true && response.data) {
        ...
        if (info.address) {
          this.userLat = info.address.lat;
          this.userLng = info.address.lng;
          this.address = info.address.landmark + ' ' + info.address.house + ' ' + info.address.address + ' ' + info.address.zipcode;
  ...
  changeStatus(value) {
    console.log(value);
    //this.status = this.changeStatusOrder;
  ...
  changeOrderStatus() {
    ...
    if (this.changeStatusOrder) {
      if (this.changeStatusOrder !== 'ongoing' && this.driverId !== '0') {
